...
export default function CreateBlogPage() {
  ...
  const checkDuplicateImage = async (filename) => {
    try {
      ...
      // In a real implementation, you'd check against existing blog images
      return false;
    } catch (error) {
  ...
  const handleChangeDropZone = async (files) => {
    ...
    for (const file of files) {
      ...
      try {
        ...
        // Get blog title and category for filename
  ...
  const handleDeleteConfirm = async (deleteType) => {
    ...
    try {
      if (deleteType === "s3") {
        // Delete from S3 bucket and all blogs
        ...
        // Trigger gallery refresh to remove deleted image
        setGalleryRefreshTrigger(prev => prev + 1);
      } else {
        // Just remove from blog (no S3 deletion)
        enqueueSnackbar("Image removed from blog!", { variant: "success" });
      }

      // Remove from files array in both cases
      setFiles((prevFiles) =>
        prevFiles.filter(
          (file) =>
      ...
      // Clear the main image if it was the deleted one
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(error?.data?.message || 'Failed to create blog post');
    }
  };
